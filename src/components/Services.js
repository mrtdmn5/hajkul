          ...
          {content.services.map((service, index) => (
            ...
            >
              <div className="absolute top-0 right-0 p-3 opacity-20 transform translate-x-1/4 -translate-y-1/4 group-hover:scale-110 group-hover:opacity-40 transition-all duration-500">
                <IconComponent name={service.icon} className="w-32 h-32 text-purple-500" />
              </div>
              
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500/20 to-blue-500/20 border border-white/10 flex items-center justify-center mb-6 relative z-10 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all duration-300">
                <IconComponent name={service.icon} className="w-7 h-7 text-cyan-400 group-hover:text-white" />
              </div>
              
              <h3 className="text-xl font-semibold text-white mb-4 font-poppins relative z-10 group-hover:text-purple-300 transition-colors">
                {service.title}
              </h3>
